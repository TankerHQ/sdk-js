...
import { utils, EncryptionV11 } from '@tanker/crypto';
  ...
  describe('stream encryption', () => {
    ...
    describe('padding with streams', () => {
      ...
      it('encrypts with auto padding by default', async () => {
        ...
        expect(EncryptionV11.getClearSize(encrypted.length)).to.equal(fiveMiB);
      ...
      it('encrypts and decrypts with no padding', async () => {
        ...
        expect(EncryptionV11.getClearSize(encrypted.length)).to.equal(almost5MiBBuffer.length);
      ...
      it('encrypts and decrypts with a padding step', async () => {
        ...
        expect(EncryptionV11.getClearSize(encrypted.length) % 500).to.equal(0);
