...
export const generateSessionTokenTests = (args: TestArgs) => {
  describe('session token (2FA)', () => {
    ...
    it('can get a session token from registerIdentity [2380K2]', async () => {
    ...
    it('cannot get a session token with a verification key [8UALZD]', async () => {
    ...
    it('can check a session token with multiple allowed methods [WZQ62Q]', async () => {
    ...
    it('fails to check a session token if the allowed_method is wrong [A80KC5]', async () => {
    ...
    it('fails to check a session token if the token is invalid [FB812H]', async () => {
      const email = 'john.smith@tanker.io';
      const verificationCode = await appHelper.getEmailVerificationCode(email);
      const token = await bobLaptop.registerIdentity({ email, verificationCode }, { withSessionToken: true });
    ...
    it('fails when using setVerificationMethod to get a session token with a preverified email [KF2J6T]', async () => {
    ...
    it('fails when using setVerificationMethod to get a session token with a preverified phone number [FN1S1V]', async () => {
      await bobLaptop.registerIdentity({ passphrase: 'Space and time are not what you think' });
    ...
    it('can use setVerificationMethod with email to get a session token [LK2XMX]', async () => {
      await bobLaptop.registerIdentity({ passphrase: 'Space and time are not what you think' });
    ...
    it('can use setVerificationMethod with phone number to get a session token [3Y7MDS]', async () => {
      await bobLaptop.registerIdentity({ passphrase: 'Space and time are not what you think' });
    ...
    it('can use verifyIdentity to get a session token when Ready [2WNCSO]', async () => {
