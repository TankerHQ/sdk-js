...
describe('TransparentSessionStore', () => {
  ...
  it('saves and finds session keys', async () => {
    ...
    expect(await sessionStore.findSessionKey(hash)).to.deep.equal({
      id: utils.toBase64(session.id),
      key: session.key,
    });
  });
  ...
  it('does not find session keys form the future', async () => {
    const nowStub = sinon.stub(sessionStore, 'now');
    stub = nowStub;
    nowStub.returns(new Date('2022-01-02T00:00').getTime());
    const session = makeSession();
...
describe('computeRecipientHash', () => {
  it('compute different hash for different lists', () => {
