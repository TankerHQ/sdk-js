...
export async function keyPublish(
  ...
  await client.sendKeyPublishBlocks(blocks).catch(e => {
    // TODO: remove when multiple re-share of same keyPublish no longer can occur on several devices
    if (!(e instanceof ServerError) || e.error.status !== 409) { // existing_equivalent_block
      throw e;
    }
  });
}
