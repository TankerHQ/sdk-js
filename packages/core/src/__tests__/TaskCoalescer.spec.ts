...
describe('TaskCoalescer', () => {
  ...
  it('forwards errors from already in-progress task', async () => {
    ...
    const originalTasks = coalescer.run(
      () => sync.promise,
      [0, 1, 2],
    );

    const coalescedTasks = coalescer.run(
      async () => { assert(false, 'Unexpected call to tasks handler'); return []; },
      [0, 1, 2],
    );

    const originalError = new InvalidArgument('This is a test error');
    sync.reject(originalError);

    // Order of rejection should not matter
    await Promise.all([
      expect(originalTasks).to.rejectedWith(originalError),
      expect(coalescedTasks).to.rejectedWith(originalError),
    ]);
  });
  ...
  it('omits unresolved ids from resulting array', async () => {
    const taskIds = [0, 1, 2];
    expect((
      await coalescer.run(async (ids) => ids.slice(1).map(id => ({ id })), taskIds)
    ).map(key => key.id)).to.deep.equal(taskIds.slice(1));
  });

  it('omits ids omitied by in-progress task', async () => {
    const sync = new PromiseWrapper<void>();
    const taskIds = [0, 1, 2];

    const originalTasks = coalescer.run(async (ids) => {
      await sync.promise;
      return ids.slice(1).map(id => ({ id }));
    }, taskIds);
    ...
    await Promise.all([
      expect((await originalTasks).map(key => key.id)).to.deep.equal(taskIds.slice(1)),
      expect((await coalescedTasks).map(key => key.id)).to.deep.equal(taskIds.slice(1)),
    ]);
  });
