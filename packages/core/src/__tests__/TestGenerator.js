...
class TestGenerator {
  ...
  makeProvisionalIdentityClaim = (parentDevice: TestDeviceCreation, user: LocalUser): TestIdentityClaim => {
    ...
    this._trustchainIndex += 1;
    const block = blockGenerator.makeProvisionalIdentityClaimBlock(user.userId, user.currentUserKey.publicKey, provisionalIdentityPrivateKeys);
    block.index = this._trustchainIndex;
  ...
  makeUserGroupAddition = (parentDevice: TestDeviceCreation, previousGroup: TestUserGroup, newMembers: Array<User>, provisionalUsers?: Array<FullPublicProvisionalIdentity>): TestUserGroup => {
    const blockGenerator = new BlockGenerator(
      this._trustchainId,
      parentDevice.testDevice.signKeys.privateKey,
      parentDevice.testDevice.id,
