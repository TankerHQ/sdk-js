...
describe('GroupUpdater', () => {
  let builder;
  let groupStore;
  let provisionalIdentityPrivateKeys;
  let provisionalIdentityPublicKeys;
  beforeEach(async () => {
    builder = await makeTrustchainBuilder();
    groupStore = await makeMemoryGroupStore();

    provisionalIdentityPrivateKeys = {
    ...
    provisionalIdentityPublicKeys = {
      ...
      tankerEncryptionPublicKey: provisionalIdentityPrivateKeys.tankerEncryptionKeyPair.publicKey,
    };
  });
  ...
  it('handles a group creation I do not belong to as a pre-registered user', async () => {
    const alice = await builder.addUserV3('alice');
  ...
  it('handles a provisional identity claim and decrypts pending group memberships', async () => {
    ...
    const provisionalIdentityWithId = {
      id: utils.toBase64(utils.concatArrays(provisionalIdentityPrivateKeys.appSignatureKeyPair.publicKey, provisionalIdentityPrivateKeys.tankerSignatureKeyPair.publicKey)),
      appEncryptionKeyPair: provisionalIdentityPrivateKeys.appEncryptionKeyPair,
      tankerEncryptionKeyPair: provisionalIdentityPrivateKeys.tankerEncryptionKeyPair,
    };
