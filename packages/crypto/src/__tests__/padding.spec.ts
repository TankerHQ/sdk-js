...
describe('Padding', () => {
  describe('padme', () => {
    it('returns a number, including for values under 1', () => {
      expect(() => padding.padme(0)).to.not.throw;
      expect(() => padding.padme(1)).to.not.throw;
      expect(() => padding.padme(-42)).to.not.throw;
    });
  ...
  describe('padClearData', () => {
    it('pads the data with a minimum padding', () => {
      ...
      expect(actual.length).to.equal(padding.minimalPadding);
  ...
  describe('removePadding', () => {
    it('throws if 0x80 is not found or the following bytes are not 0x00', () => {
      expect(() => padding.removePadding(new Uint8Array(0))).to.throw();
      expect(() => padding.removePadding(new Uint8Array([0x74, 0x72, 0x75, 0x65]))).to.throw();
      expect(() => padding.removePadding(new Uint8Array([0x74, 0x72, 0x75, 0x65, 0x80, 0x42]))).to.throw();
      expect(() => padding.removePadding(new Uint8Array([0x74, 0x72, 0x75, 0x65, 0x80, 0x00, 0x42, 0x00]))).to.throw();
    });
