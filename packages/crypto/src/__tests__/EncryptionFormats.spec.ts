...
import { EncryptionV11, EncryptionV10, EncryptionV9 } from '../EncryptionFormats/TransparentEncryption';
...
describe('Simple Encryption', () => {
  ...
  const generatePaddedTests = (encryptor: Encryptor, testVectors: Array<TestVector>, { encrypt, decrypt, overhead = () => encryptor.overhead }: PaddedEncryptorOperations) => {
    ...
    it('computes clear and encrypted sizes', () => {
      const { getClearSize, getEncryptedSize } = encryptor;
      const clearSize = getClearSize(testVectors[0]!.encryptedData.length);
      const encryptedSize = getEncryptedSize(testVectors[0]!.clearData.length);
    ...
    it('encryptedSize should have a minimal value', async () => {
      for (const clearSize of [0, 1, 8, 9]) {
        ...
        expect(encryptor.getEncryptedSize(clearSize, Padding.AUTO)).to.equal(minimalPadding + overhead());

        const key = random(tcrypto.SYMMETRIC_KEY_SIZE);
        expect((await encrypt(key, new Uint8Array(clearSize), Padding.AUTO)).length).to.equal(minimalPadding + overhead());
      }
    });
    ...
    it('encryptedSize should use the padme algorithm in auto padding', async () => {
      ...
      for (const [clearSize, paddedSize] of paddedWithAuto) {
        ...
        expect(encryptor.getEncryptedSize(clearSize, Padding.AUTO)).to.equal(paddedSize + overhead());

        const key = random(tcrypto.SYMMETRIC_KEY_SIZE);
        expect((await encrypt(key, new Uint8Array(clearSize), Padding.AUTO)).length).to.equal(paddedSize + overhead());
      }
    });
    ...
    it('encryptedSize should use the paddingStep parameter correctly', async () => {
      ...
      for (const [clearSize, paddedSize] of paddedToStepFive) {
        expect(encryptor.getEncryptedSize(clearSize, 5)).to.equal(paddedSize + overhead());

        const key = random(tcrypto.SYMMETRIC_KEY_SIZE);
        expect((await encrypt(key, new Uint8Array(clearSize), 5)).length).to.equal(paddedSize + overhead());
      }
    });
  };
  ...
  describe('EncryptionFormatV11', () => {
    const encrypt = (k: Uint8Array, d: Uint8Array, padding: number | Padding) => processWithStream(() => new EncryptionStreamV11(random(tcrypto.SESSION_ID_SIZE), k, padding), d);
    generateCommonTests(EncryptionV11, testVectorsV11, {
      encrypt: (k: Uint8Array, d: Uint8Array) => encrypt(k, d, Padding.AUTO),
      decrypt: (k: Uint8Array, d: Uint8Array) => processWithStream(() => new DecryptionStream(() => k), d),
    });
    generatePaddedTests(EncryptionV11, testVectorsV11, {
      encrypt,
      decrypt: (k: Uint8Array, d: Uint8Array) => processWithStream(() => new DecryptionStream(() => k), d),
      overhead: () => EncryptionV11.overhead + EncryptionV11.chunkOverhead,
    });
  });
});
